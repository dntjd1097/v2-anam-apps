// 충전 페이지 로직
console.log('Busan Card charge page loaded');
...
async function loadBlockchainBalance() {
  try {
    ...
    document.getElementById('blockchain-balance').innerHTML = `
      <p style="text-align: center; color: #e74c3c;">Unable to load balance information.</p>
    `;
  }
}
...
async function processCharge() {
  ...
  if (amount < 1 || amount > 1000) {
    alert('Charge amount must be between $1 and $1,000.');
    return;
  }
...
function showSuccessMessage() {
  ...
  successDiv.innerHTML = `
    ...
    <div class="success-text">Charge completed!</div>
  `;
