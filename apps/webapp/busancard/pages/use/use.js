// 사용 페이지 로직
console.log('Busan Card use page loaded');
...
function refreshQR() {
  ...
  setTimeout(() => {
    ...
    // 성공 메시지
    showToast('QR code has been refreshed');
...
function resetTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (remainingTime <= 0) {
      clearInterval(timerInterval);
      showToast('QR code has expired. Please refresh.');
    ...
    document.querySelector('.info-subtext').textContent = 
      `Valid for: ${minutes} min ${seconds.toString().padStart(2, '0')} sec`;
  }, 1000);
}
