...
window.App = {
  // 앱 시작 시 호출 (최초 1회)
  onLaunch(options) {
    console.log("Mini app launched:", options);
...
class BitcoinAdapter extends CoinAdapter {
  ...
  async generateWallet() {
    try {
      ...
      console.error("Bitcoin wallet creation failed:", error);
      throw new Error(
        "Failed to create Bitcoin wallet. Please check if bitcoinjs-lib is loaded."
      );
    }
  }
  ...
  async importFromMnemonic(mnemonic) {
    try {
      ...
      console.error("Bitcoin mnemonic recovery failed:", error);
      throw new Error("Failed to recover Bitcoin wallet from mnemonic: " + error.message);
    }
  }
  ...
  async importFromPrivateKey(privateKey) {
    try {
      ...
      console.error("Bitcoin private key recovery failed:", error);
      throw new Error("Failed to recover Bitcoin wallet from private key: " + error.message);
    }
  }
  ...
  async getUTXOs(address) {
    try {
      ...
      console.error("UTXO query failed:", error);
      return [];
    }
  }
  ...
  async getFeeEstimates() {
    try {
      ...
      console.error("Fee query failed:", error);
      return {
        fastestFee: 20,
        halfHourFee: 10,
        hourFee: 5,
  ...
  async getGasPrice() {
    try {
      ...
      console.error("Fee rate query failed:", error);
      // 기본값 반환 (sat/vByte)
      return {
        low: "5",
        medium: "10",
